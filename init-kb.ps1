...
.DESCRIPTION
    ...
    特色：
      ...
      - 衝突確認守門：測試損壞、功能飄移或規格衝突時先暫停並向使用者確認
...
實作中若出現以下狀況，必須立即暫停擴大修改，先整理證據並向使用者確認：

- 既有測試因本次修改轉紅，或修 A 導致 B / C 測試失敗。
- 需要改既有測試期待、snapshot、fixture、mock 或驗收條件才可通過。
- 實作與 OpenSpec、quickref、既有商務規則或使用者原始需求衝突。
- 發現功能飄移、單元測試飄移、跨模組耦合或未列入計劃的行為改變。

回報時列出失敗測試/命令、受影響行為、疑似根因、已知證據與可選方向；不得自行接受新行為或修改測試期待。只有明顯屬於同一計劃內的語法錯、測試 setup 錯誤或 mock 漏補，且不改變行為契約時，才可修正後回報。

## 回報

讀完上述後向用戶回報：涉及模組、命中主題（topic slug）、命中陷阱編號、操作守門摘要（preflight / pending repair / 替代讀取方式）、TDD 入口、SOLID / 範圍約束、衝突確認條件、待探索範圍。
...
4. **衝突停止線**：哪些既有測試、行為契約、OpenSpec requirement、商務規則或跨模組相依一旦受影響，必須先暫停並向使用者確認。

若計劃需要擴大到未列入的檔案、跨模組設計或行為契約變更，必須先列為待確認事項，不得直接實作。
若預期可能需要修改既有測試期待、snapshot、fixture、mock 或驗收條件，必須先列為待確認事項，不得在計劃中預設接受新行為。
...
| 衝突確認條件 | （測試損壞、功能飄移、規格衝突、需改測試期待時如何停下來確認） |
...
以 `[探索]` / `[TDD]` / `[新增]` / `[修改]` / `[測試]` / `[衝突確認]` / `[SOLID檢查]` / `[知識庫]` 標籤列出步驟。
...
8. **工程約束回顧**：確認本次程式碼修改符合 TDD / SOLID / 可驗證範圍。
  ...
  - 衝突確認：說明是否曾出現測試損壞、功能飄移、規格衝突或需修改測試期待；若有，列出已回報使用者的證據、使用者決策與後續更新的規格/測試/知識庫。若沒有，明確說明未發現計劃外行為變更。
...
**程式碼修改守門**：TDD 優先、SOLID 約束、可驗證範圍；測試損壞 / 功能飄移 / 規格衝突時先暫停確認；詳見 `copilot-instructions.md`
...
4. 若曾出現測試損壞、功能飄移、規格衝突或需修改測試期待，列出使用者確認結果與後續處理
5. 若本任務有失敗或重複嘗試，執行 `repair-status` / `repair-health`
6. `node .vscode/knowledge/scripts/kb.mjs rebuild`
7. `node .vscode/knowledge/scripts/kb.mjs finish-check`（必須 0 errors）
...
rules:
  ...
  proposal:
    ...
    - 若可能出現測試損壞、功能飄移、規格衝突或需修改既有測試期待，必須列為使用者確認事項
  specs:
...
artifacts:
  ...
  - id: proposal
    ...
    instruction: |
      ...
      - **衝突確認**：列出哪些既有測試、行為契約或商務規則一旦受影響，必須先暫停並向使用者確認
  ...
  - id: design
    ...
    instruction: |
      ...
      - **衝突停止線**：說明若實作造成測試損壞、功能飄移、規格衝突或需改測試期待時，應停止在哪裡並回報哪些證據
  ...
  - id: tasks
    ...
    instruction: |
      ...
      **必要群組**：
      ...
      4. 衝突確認回顧（若測試損壞、功能飄移、規格衝突或需改測試期待，列出使用者決策；未發生也需註明）
      5. 測試驗證（**此群組不可省略**）
      6. 知識庫更新（**此群組不可省略**）
...
artifacts:
  - id: proposal
    ...
    instruction: |
      ...
      - **衝突確認**：列出此修復可能影響的既有測試、行為契約或商務規則；若需改測試期待，必須先確認
